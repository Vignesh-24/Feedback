...
<table>
    ...
    <tr>
        <th colspan="5">Name of the faculty :<?php echo $_SESSION['name']; ?></th>
    </tr>
    <tr>
  ...
  while($list = mysqli_fetch_assoc($res)){
    ...
    $section=$list['sec'];
      ...
      if($type=='LAB'){
      $res1=mysqli_query($scon,"select AVG(l1),AVG(l2),AVG(l3),AVG(l4),AVG(l5),AVG(l6),AVG(l7),AVG(l8),AVG(l9),AVG(l10)from lab where sub_code='$username' and staff_id='$category' and section='$section' and batch='$bat'");
    $row=mysqli_fetch_array($res1); $percent=($row['AVG(l1)']+$row['AVG(l2)']+$row['AVG(l3)']+$row['AVG(l4)']+$row['AVG(l5)']+$row['AVG(l6)']+$row['AVG(l7)']+$row['AVG(l8)']+$row['AVG(l9)']+$row['AVG(l10)'])*2;
      ...
      $res1=mysqli_query($scon,"select AVG(t1),AVG(t2),AVG(t3),AVG(t4),AVG(t5),AVG(t6),AVG(t7),AVG(t8),AVG(t9),AVG(t10),AVG(t11),AVG(t12),AVG(t13),AVG(t14),AVG(t15),AVG(t16),AVG(t17),AVG(t18),AVG(t19),AVG(t20) from theory where sub_code='$username' and staff_id='$category' and section='$section' and batch='$bat'");
...
?>
    ...
    <tr>
    <td><?php echo $username; ?></td><td><?php echo $row2['subname']; ?></td><td><?php echo $row2['sem']; ?></td><td><?php echo $section ?></td><td><?php echo number_format($percent,2); ?></td></tr><br><br>
    <?php } ?></table>
        </div>
    <br><br>
    <div align="center">
